...
class EntryTest extends TestCase
{
    ...
    public function testAllMethodBackwardCompatibility()
    {
        $this->entry->setClient($this->createMockClient([
            'happycat' => MockEntry::withSys('happycat'),
            'garfield' => MockEntry::withSys('garfield'),
        ...
        // Test that all() works exactly as before (default behavior)
        $allFields = $this->entry->all();
        $this->assertArrayHasKey('bestFriend', $allFields);
        $this->assertArrayHasKey('enemy', $allFields);
        $this->assertSame('happycat', $allFields['bestFriend']->getId());
        $this->assertSame('garfield', $allFields['enemy']->getId());

        // Test with explicit defaults - should behave identically
        $allFieldsExplicit = $this->entry->all(null, 1, false, false);
        $this->assertEquals($allFields, $allFieldsExplicit);
    }

    public function testAllMethodWithNestedAssetResolution()
    {
        $this->entry->setClient($this->createMockClient([
            'happycat' => MockEntry::withSys('happycat'),
            'garfield' => MockEntry::withSys('garfield'),
        ...
        // Test with nested asset resolution enabled
        $allFieldsWithAssets = $this->entry->all(null, 2, false, true);
        
        // Should still resolve entry links as before
        $this->assertArrayHasKey('bestFriend', $allFieldsWithAssets);
        $this->assertArrayHasKey('enemy', $allFieldsWithAssets);
        $this->assertSame('happycat', $allFieldsWithAssets['bestFriend']->getId());
        $this->assertSame('garfield', $allFieldsWithAssets['enemy']->getId());
    }

    public function testGetMethodBackwardCompatibility()
    {
        $this->entry->setClient($this->createMockClient([
            'happycat' => MockEntry::withSys('happycat'),
        ]));

        // Test that get() works exactly as before (default behavior)
        $bestFriend = $this->entry->get('bestFriend');
        $this->assertSame('happycat', $bestFriend->getId());

        // Test with explicit defaults - should behave identically
        $bestFriendExplicit = $this->entry->get('bestFriend', null, 1, false);
        $this->assertEquals($bestFriend, $bestFriendExplicit);
    }

    public function testGetMethodWithNestedAssetResolution()
    {
        $this->entry->setClient($this->createMockClient([
            'happycat' => MockEntry::withSys('happycat'),
        ]));
        ...
        // Test with nested asset resolution enabled
        $bestFriendWithAssets = $this->entry->get('bestFriend', null, 2, true);
        
        // Should still resolve entry link as before
        $this->assertSame('happycat', $bestFriendWithAssets->getId());
    }

    public function testResolveLinksInNestedResourcesMethod()
    {
        $this->entry->setClient($this->createMockClient([
            'happycat' => MockEntry::withSys('happycat'),
        ]));

        // Test the new method exists and can be called (using reflection)
        $reflection = new \ReflectionClass($this->entry);
        $method = $reflection->getMethod('resolveLinksInNestedResources');
        $method->setAccessible(true);

        // Test with Entry object
        $mockEntry = MockEntry::withSys('test');
        $result = $method->invoke($this->entry, $mockEntry, 1, false, null, false);
        $this->assertSame($mockEntry, $result);

        // Test with array containing Entry
        $entryArray = [$mockEntry];
        $result = $method->invoke($this->entry, $entryArray, 1, false, null, false);
        $this->assertSame($entryArray, $result);

        // Test with non-resource value
        $result = $method->invoke($this->entry, 'string value', 1, false, null, false);
